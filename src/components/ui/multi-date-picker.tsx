        ...
        {calendarDays.map((item, idx) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              className={`
                h-8.5 text-xs font-semibold rounded-full transition-all flex items-center justify-center relative select-none cursor-pointer
                ${
                  item.isSelected
                    ? "bg-[#0084c7] text-white font-bold shadow-xs hover:bg-[#0077b6] scale-100 ring-2 ring-sky-300"
                    : item.isToday
            ...
            >
              <span>{item.day}</span>
              {item.isSelected && (
                <span className="absolute -top-0.5 -right-0.5 h-3.5 w-3.5 bg-white text-[#0084c7] rounded-full flex items-center justify-center text-[9px] shadow-2xs font-black">
                  ✓
                </span>
              )}
            </button>
