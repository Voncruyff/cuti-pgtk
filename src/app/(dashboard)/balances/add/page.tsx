...
import { addMultipleLeaveBalanceAction } from "@/actions/balance-actions";
        ...
        <BalanceActivityCard
          ...
          actionButton={
            <Button
              ...
              className="rounded-full gap-1.5 h-9 px-4 text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs"
            >
        ...
        <DialogContent onClose={() => setIsModalOpen(false)} className="max-w-md">
          ...
          <form onSubmit={handleSubmitAddBalance} className="space-y-4 pt-2">
            ...
            <div className="space-y-1">
              <Label htmlFor="transactionDate" required className="text-xs font-semibold text-slate-700">
                Tanggal Penambahan Saldo:
              </Label>
              <Input
                id="transactionDate"
                type="date"
            ...
            <div className="space-y-3 bg-slate-50/80 p-3.5 rounded-lg border border-slate-200">
              ...
              <div className="flex items-center justify-between gap-3 bg-white p-2.5 rounded-md border border-slate-200">
                <div className="text-xs">
                  <span className="font-semibold text-blue-900 block">Cuti Tahunan</span>
                  <span className="text-[10px] text-slate-500">Saldo saat ini: {selectedEmployee?.balances.annual} hr</span>
                </div>
                <div className="relative w-28">
                  <Input
                    ...
                    disabled={isPending}
                    className="h-8 text-xs pr-10 font-bold font-mono text-right"
                  />
                  <span className="absolute right-2.5 top-2 text-[11px] text-slate-400">hr</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-3 bg-white p-2.5 rounded-md border border-slate-200">
                <div className="text-xs">
                  <span className="font-semibold text-purple-900 block">Cuti Besar</span>
                  <span className="text-[10px] text-slate-500">Saldo saat ini: {selectedEmployee?.balances.longLeave} hr</span>
                </div>
                <div className="relative w-28">
                  <Input
                    type="number"
                    ...
                    placeholder="0"
                    onChange={(e) => setLongLeaveAmount(Number(e.target.value) || 0)}
                    disabled={isPending}
                    className="h-8 text-xs pr-10 font-bold font-mono text-right"
                  />
                  <span className="absolute right-2.5 top-2 text-[11px] text-slate-400">hr</span>
              ...
              {selectedEmployee?.category?.toUpperCase() !== "PELAKSANA" && (
                <div className="flex items-center justify-between gap-3 bg-white p-2.5 rounded-md border border-slate-200">
                  <div className="text-xs">
                    <span className="font-semibold text-amber-900 block">Inhaldagen</span>
                    <span className="text-[10px] text-slate-500">Saldo saat ini: {selectedEmployee?.balances.inhaldagen} hr</span>
                  </div>
                  <div className="relative w-28">
                    <Input
                      type="number"
                      ...
                      placeholder="0"
                      onChange={(e) => setInhaldagenAmount(Number(e.target.value) || 0)}
                      disabled={isPending}
                      className="h-8 text-xs pr-10 font-bold font-mono text-right"
                    />
                    <span className="absolute right-2.5 top-2 text-[11px] text-slate-400">hr</span>
              ...
              {/* Total Ditambahkan */}
              <div className="flex justify-between items-center pt-2 border-t border-slate-200 text-xs font-semibold">
                <span className="text-slate-700">Total Ditambahkan:</span>
            ...
            <DialogFooter className="gap-2 pt-2">
              ...
              <Button
                ...
                className="rounded-full h-9 text-xs font-bold gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs"
              >
                {isPending ? (
                  <>
