...
export function KomponenHalamanStasiun() {
  ...
  const handleEdit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await updateStationAction(editingId, {
        code: editKode.trim().toUpperCase(),
        name: editNama.trim().toUpperCase(),
    ...
    <>
      ...
      <div className="flex items-center gap-1 p-1 bg-slate-100/80 rounded-lg border border-slate-200/80 w-fit mb-4">
        ...
        >
          ...
          Master Bagian
          <span className="h-4.5 px-1.5 text-[10px] bg-slate-200 text-slate-700 font-semibold rounded-md flex items-center justify-center">
            {bagian.length}
          </span>
        </Link>
      </div>
