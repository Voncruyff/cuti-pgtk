...
import {
  ...
  type PilihanBagian,
  type PilihanStasiun,
} from "@/components/fitur/master-karyawan/tabel-karyawan";
import {
  ModalTambahKaryawan,
  ModalEditKaryawan,
  ModalHapusKaryawan,
} from "@/components/fitur/master-karyawan/modal-karyawan";
import {
...
import type { SessionUser } from "@/types/autentikasi";
...
export function KomponenMasterKaryawan({ user }: KomponenMasterKaryawanProps) {
  ...
  const muatData = async () => {
    ...
    try {
      ...
      if (res.departments) {
        setBagian(res.departments as PilihanBagian[]);
        if (user.role === "ADMIN_BAGIAN" && res.departments.length > 0) {
          setFilterBagian(res.departments[0].id);
        }
      }
  ...
  const handleTambah = (e: React.FormEvent) => {
    e.preventDefault();
    if (!canManage) {
      toast.error("Anda tidak memiliki hak akses untuk menambah karyawan.");
      return;
    }
  ...
  const handleBukaEdit = (item: ItemKaryawan) => {
    ...
    const matchSt = stasiun.find(
      (s) => s.id === item.stationId || (item.stasiun && s.name.toLowerCase() === item.stasiun.toLowerCase())
    );
  ...
  const handleHapus = () => {
    if (!canManage || !hapusKaryawan) return;
    startTransition(async () => {
  ...
  return (
    <>
      ...
      <TabelKaryawan
        ...
        onTambah={() => {
          if (!canManage) return;
          if (tabKategori === "PELAKSANA") setKategori("PELAKSANA");
          else setKategori("PIMPINAN");
      ...
      {/* Modal Tambah, Edit, Hapus hanya dirender jika memiliki hak akses kelola */}
      {canManage && (
        <>
          <ModalTambahKaryawan
            ...
            onUbahNip={setNip}
            onUbahNama={setNama}
            onUbahJabatan={setJabatan}
            onUbahKategori={setKategori}
            onUbahBagianId={setBagianId}
            onUbahStasiunId={setStasiunId}
            onUbahTglPengangkatan={setTglPengangkatan}
          ...
          <ModalEditKaryawan
            ...
            stasiunId={editStasiunId}
            tglPengangkatan={editTglPengangkatan}
            onUbahNip={setEditNip}
            onUbahNama={setEditNama}
            onUbahJabatan={setEditJabatan}
            onUbahKategori={setEditKategori}
            onUbahBagianId={setEditBagianId}
            onUbahStasiunId={setEditStasiunId}
            onUbahTglPengangkatan={setEditTglPengangkatan}
            onSubmit={handleEdit}
