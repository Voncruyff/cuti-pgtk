...
import { getDepartmentsAction, getStationsForDepartmentAction } from "@/actions/employee-actions";
...
export default function ReportsPage() {
  ...
  type BalanceSortField = "no" | "nip" | "nama" | "jabatan" | "bagian" | "stasiun" | "cutiTahunan" | "cutiBesar" | "inhaldagen" | "totalSaldo";
  ...
  const sortedBalanceItems = useMemo(() => {
    return [...filteredBalanceItems].sort((a, b) => {
      ...
      } else if (balanceSortField === "jabatan") {
  ...
  const handleExportCSV = () => {
    try {
      ...
      if (activeTab === "BALANCES") {
        const headers = ["No", "NIP", "Nama Karyawan", "Jabatan", "Bagian", "Stasiun", "Cuti Tahunan", "Cuti Besar", "Inhaldagen", "Total Saldo"];
        ...
        const headers = ["No", "NIP", "Nama Karyawan", "Bagian", "Stasiun", "Tgl Transaksi", "Uraian", "Tanggal Cuti", "Cuti Tahunan", "Cuti Besar", "Inhaldagen", "Total Hari"];
  ...
  return (
    <div className="space-y-6 w-full pb-12 print:p-0 print:m-0 print:max-w-none">
      ...
      <div className="print:hidden flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-200/80 pb-3">
        ...
        <div className="flex items-center gap-2 self-start sm:self-center">
          <Button
            ...
            size="sm"
            onClick={handlePrint}
            className="rounded-full gap-1.5 h-9 px-4 text-xs font-semibold bg-white shadow-2xs hover:bg-slate-50"
          >
          ...
          <Button
            ...
            className="rounded-full gap-1.5 h-9 px-4 text-xs font-bold bg-emerald-700 hover:bg-emerald-800 text-white shadow-xs"
          >
      ...
      <div className="print:hidden flex items-center gap-2 p-1 bg-slate-100/70 rounded-full w-fit border border-slate-200/80">
        <button
          type="button"
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50/80 text-[11px]">
                    ...
                    <TableHead
                      className="font-bold cursor-pointer select-none hover:bg-slate-100 transition-colors group"
                      onClick={() => handleBalanceSort("jabatan")}
                      title="Urutkan Jabatan"
                    >
                      <div className="inline-flex items-center gap-1">
                        <span>Jabatan</span>
                        {renderSortIcon(balanceSortField, "jabatan", balanceSortOrder)}
                      </div>
                    </TableHead>
                    <TableHead
                      className="font-bold cursor-pointer select-none hover:bg-slate-100 transition-colors group"
                ...
                <TableBody>
                  {sortedBalanceItems.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={10} className="h-24 text-center text-slate-500 text-xs">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50/80 text-[11px]">
                    ...
                    <TableHead
                      className="font-bold cursor-pointer select-none hover:bg-slate-100 transition-colors group"
                      onClick={() => handleUsageSort("stasiun")}
                      title="Urutkan Stasiun"
                    >
                      <div className="inline-flex items-center gap-1">
                        <span>Stasiun</span>
                        {renderSortIcon(usageSortField, "stasiun", usageSortOrder)}
                      </div>
                    </TableHead>
                    <TableHead
                      className="font-bold cursor-pointer select-none hover:bg-slate-100 transition-colors group"
                ...
                <TableBody>
                  {sortedUsageItems.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={12} className="h-24 text-center text-slate-500 text-xs">
                    ...
                    sortedUsageItems.map((item, idx) => (
                      <TableRow key={item.id} className="hover:bg-slate-50/50">
                        ...
                        {/* Tahunan */}
                        <TableCell className="text-center font-mono text-xs font-bold text-blue-700 tabular-nums">
                          {item.annualDays > 0 ? item.annualDays : <span className="text-slate-300">-</span>}
                        </TableCell>

                        {/* Besar */}
                        <TableCell className="text-center font-mono text-xs font-bold text-purple-700 tabular-nums">
                          {item.longLeaveDays > 0 ? item.longLeaveDays : <span className="text-slate-300">-</span>}
                        </TableCell>
