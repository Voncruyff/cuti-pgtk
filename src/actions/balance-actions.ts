...
export async function addMultipleLeaveBalanceAction(
  ...
  try {
    ...
    // Update mockDb transactions for ledger
    try {
      const leaveTypes = mockDb.getLeaveTypes();
      ...
      if (annualAmount > 0 && annualLt) {
        mockDb.addTransaction({
          employeeId,
          leaveTypeId: annualLt.id,
          transactionType: "ADD_BALANCE",
          transactionDate: new Date(transactionDate),
          amount: Number(annualAmount),
      ...
      if (longLeaveAmount > 0 && longLt) {
        mockDb.addTransaction({
          employeeId,
          leaveTypeId: longLt.id,
          transactionType: "ADD_BALANCE",
          transactionDate: new Date(transactionDate),
          amount: Number(longLeaveAmount),
          description: effectiveDesc,
          notes: notes || null,
          createdById: user.id,
        });
      ...
      if (inhaldagenAmount > 0 && inhalLt) {
        mockDb.addTransaction({
          employeeId,
          leaveTypeId: inhalLt.id,
          transactionType: "HOLIDAY_COMPENSATION",
          transactionDate: new Date(transactionDate),
          amount: Number(inhaldagenAmount),
          description: effectiveDesc,
          notes: notes || null,
          createdById: user.id,
        });
  ...
  try {
    ...
    if (employee) {
      try {
        ...
        console.warn("Querying aktivitas_saldo fallback to mockDb:", sqlErr);
      }
    }

    const rawTx = mockDb.getTransactions(employeeId);
    // Filter only positive additions
    const addTx = rawTx.filter((t) => t.amount > 0);

    const items: BalanceAddHistoryItem[] = addTx.map((t) => {
      const isAnnual = t.leaveType?.code === "ANNUAL";
      const isLong = t.leaveType?.code === "LONG_LEAVE";
      const isInhal = t.leaveType?.code === "INHALDAGEN";

      return {
        id: t.id,
        employeeId: t.employeeId,
        transactionDate: t.transactionDate.toISOString(),
        description: t.description || "Penambahan Saldo",
        annualDays: isAnnual ? t.amount : 0,
        longLeaveDays: isLong ? t.amount : 0,
        inhaldagenDays: isInhal ? t.amount : 0,
        totalDays: t.amount,
        notes: t.notes || null,
        createdByName: t.createdBy?.fullName || t.createdBy?.username || "Admin",
